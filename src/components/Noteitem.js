...
export default function Noteitem(props) {
    ...
    const {createNote,setcreateNote,update,setupdate,render}=context
    const { title, desc, tag,edited, date,id,num ,confirmAlert, setconfirmAlert } = props
    let d = new Date(date)
    let updatingNote=()=>{
        setcreateNote(true)
        setupdate({
    ...
    return (
        ...
        <div className={`card card-cascade wider rounded animated ${render===id? "slideInLeft":""} faster`} style={{ width: '250px',maxHeight:"400px" }}>
            <div className="view view-cascade gradient-card-header  rounded p-2 text-white" style={{ backgroundColor: "#345B63" }}>
                ...
                <a className="position-absolute" style={{ top: "5px", right: "10px" }}   href={null} onClick={}><i className="lni lni-star" style={{ color: "white" }}></i></a>
